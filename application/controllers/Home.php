...
class Home extends CI_Controller
{
    ...
    {
        if ($this->session->userdata('attempt') > 7) {
            if (time() - $this->session->userdata('time') > 86400) {
        ...
        $page_data['page_name']     = 'phone_login';
        $page_data['title']         = get_phrase('phone_login');
        $this->load->view('frontend/index', $page_data);
    }
    ...
    public function otp_verification()
    {
        ...
        $page_data['title']         =   get_phrase('otp_verification');
        $this->load->view('frontend/index', $page_data);
    }
    ...
    public function sign_up_using_phone()
    {
        $page_data['page_name']     =   'sign_up_using_phone';
        $page_data['title']         =   get_phrase('sign_up_using_phone_number');
        $this->load->view('frontend/index', $page_data);
    }
    ...
    function contact_us($listing_type = "")
    {
        ...
        if ($listing_type == 'restaurant') {
            ...
            $data['phone'] = $listing_details['phone'];

            $message = 'Hello, \n We have recieved your table booking request for ' . $data['date'] . ' @ ' . $data['time'] . ' with ' . $data['adult_guests_for_booking'] . ' adults and ' . $data['child_guests_for_booking'] . ' clidren';
            if ($data['date'] != "" && $data['time'] != "") {
                $this->frontend_model->restaurant_booking();
                $this->email_model->restaurant_booking_mail($data);
                send_sms($message, $this->session->userdata('phone'));
                send_sms('Booking Table Request For ' . $data['date'] . ' @ ' . $data['time'] . ' with ' . $data['adult_guests_for_booking'] . ' adults and ' . $data['child_guests_for_booking'] . ' clidren', $data['phone']);
            } else {
            ...
            $data['to'] = $listing_details['email'];
            $data['phone'] = $listing_details['phone'];
            $message = 'Hello, \n We have recieved your hotel booking request from ' . $data['book_from'] . ' - ' . $data['book_to'] . ' with ' . $data['adult_guests_for_booking'] . ' adults and ' . $data['child_guests_for_booking'] . ' clidren and room type is ' . $data['room_type'];
            if ($data['book_from'] != "" && $data['book_to'] != "" && $data['room_type'] != "") {
                $this->frontend_model->hotel_booking();
                $this->email_model->hotel_booking_mail($data);
                send_sms($message, $this->session->userdata('phone'));
                send_sms('Hotel Room Booking Request From ' . $data['book_from'] . ' - ' . $data['book_to'] . ' with ' . $data['adult_guests_for_booking'] . ' adults and ' . $data['child_guests_for_booking'] . ' clidren and room type is ' . $data['room_type'], $data['phone']);
            } else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
            ...
            $data['to'] = $listing_details['email'];
            $data['phone'] = $listing_details['phone'];
            $message = 'Hello, \n We have recieved your parlour booking request for ' . $data['date'] . ' @ ' . $data['time'] . ' for ' . $data['service'];
            if ($data['date'] != "" && $data['time'] != "") {
                $this->frontend_model->beauty_service();
                $this->email_model->beauty_service_mail($data);
                send_sms($message, $this->session->userdata('phone'));
                send_sms('Parlour Booking Request For ' . $data['date'] . ' @ ' . $data['time'] . ' for ' . $data['service'], $data['phone']);
            } else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
            ...
            if ($data['name'] != "" && $data['message'] != "") {
                ...
                send_sms('Hello, This is ' . $data['name'] . ' with Message: ' . $data['message'], $data['phone']);
            } else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
    ...
    function search($page_number = 1)
    {
        ...
        if ($search_string == "" && $selected_category_id == "" && $selected_city_id == "") {
            redirect('home/listings', 'refresh');
        }

        $all_listings = $this->frontend_model->search_listing_all_rows($search_string, $selected_category_id, $selected_city_id);
        $listings = $this->frontend_model->search_listing($search_string, $selected_category_id, $page_number, $selected_city_id);
        $geo_json = $this->make_geo_json_for_map($listings);
