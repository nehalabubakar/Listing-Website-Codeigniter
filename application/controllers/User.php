...
class User extends CI_Controller
{
	...
	public function stripe_checkout($package_id = "")
	{
		...
		// echo $page_data['package_details']['price'];
		$this->payment_success('stripe', $page_data['user_details']['id'], $page_data['package_details']['id'], $page_data['package_details']['price']);
	...
	function payment_success($payment_method = "", $user_id = "", $package_id = "", $paid_amount = "")
	{
		...
		// echo 'hello Nehal';
		$response = $this->crud_model->create_package_purchase_history($payment_method, $user_id, $package_id, $paid_amount);
		if ($response == true) {
			$this->session->set_flashdata('flash_message', get_phrase('payment_success'));
		} else {
			$this->session->set_flashdata('flash_message', get_phrase('an_error_occurred'));
		}
		redirect(site_url('user/purchase_history'), 'refresh');
	}

	// function create_package_history($payment_method, $package_id, $user_id, $paid_amount)
	function create_package_history()
	{
