...
class Frontend_model extends CI_Model
{
    ...
    function hotel_booking()
    {
        ...
        $fromdate = str_replace("-", "/", $dates[0]); //DateTime::createFromFormat('m-d-y', trim($dates[0]));
        $todate = str_replace("-", "/", $dates[1]); //DateTime::createFromFormat('m-d-y', trim($dates[1]));


        $data['booking_date']             = strtotime($fromdate) . ' - ' . strtotime($todate);
        $additional_data['adult_guests']  = $this->input->post('adult_guests_for_booking');
        $additional_data['child_guests']  = $this->input->post('child_guests_for_booking');
    ...
    function search_listing($search_string = '', $selected_category_id = '', $page_number = 1, $selected_city_id)
    {
        if ($page_number <= 1) :
            $starting_value = 0;
        else :
    ...
    function search_listing_all_rows($search_string = '', $selected_category_id = '', $selected_city_id = '')
    {
        if ($search_string != "") {
            $this->db->like('name', $search_string);
            $this->db->or_like('description', $search_string);
