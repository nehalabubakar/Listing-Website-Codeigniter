  ...
  function add_category()
  {
    ...
    if ($data['parent'] == 0) {
      if ($_FILES['category_thumbnail']['name'] == "") {
        ...
        move_uploaded_file($_FILES['category_thumbnail']['tmp_name'], 'mobile/uploads/category_thumbnails/' . $data['thumbnail']);
      }
    }
...
return $this->db->get('listing');
  ...
  function add_listing()
  {
    ...
    if ($_FILES['listing_thumbnail']['name'] == "") {
      ...
      move_uploaded_file($_FILES['listing_thumbnail']['tmp_name'], 'mobile/uploads/listing_thumbnails/' . $data['listing_thumbnail']);
    }
    ...
    if ($_FILES['listing_cover']['name'] == "") {
      ...
      move_uploaded_file($_FILES['listing_cover']['tmp_name'], 'mobile/uploads/listing_cover_photo/' . $data['listing_cover']);
    }

    foreach ($_FILES['listing_images']['tmp_name'] as $listing_image) {
      if ($listing_image != "") {
        $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
        move_uploaded_file($listing_image, 'mobile/uploads/listing_images/' . $random_identifier);
  ...
  function add_listing_type_wise_details($listing_type = "", $listing_id = "")
  {
    ...
    if ($listing_type == 'hotel') {
      ...
      foreach ($_FILES['room_image']['tmp_name'] as $room_image) {
        if ($room_image != "") {
          $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
          move_uploaded_file($room_image, 'mobile/uploads/hotel_room_images/' . $random_identifier);
      ...
      foreach ($_FILES['menu_image']['tmp_name'] as $menu_image) {
        if ($menu_image != "") {
          $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
          move_uploaded_file($menu_image, 'mobile/uploads/restaurant_menu_images/' . $random_identifier);
          array_push($listing_photos, $random_identifier);
        }
      }
      ...
      foreach ($_FILES['service_image']['tmp_name'] as $service_image) {
        if ($service_image != "") {
          $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
          move_uploaded_file($service_image, 'mobile/uploads/beauty_service_images/' . $random_identifier);
          array_push($listing_photos, $random_identifier);
        }
      }
      ...
      foreach ($_FILES['product_image']['tmp_name'] as $product_image) {
        if ($product_image != "") {
          $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
          move_uploaded_file($product_image, 'mobile/uploads/product_images/' . $random_identifier);
          array_push($listing_photos, $random_identifier);
        }
      }
  ...
  function website_images_uploader($image_type = "")
  {
    if ($image_type == 'banner_image') {
      move_uploaded_file($_FILES['banner_image']['tmp_name'], 'mobile/uploads/system/home_banner.jpg');
    }
  ...
  public function add_blog()
  {
    ...
    move_uploaded_file($_FILES['blog_thumbnail']['tmp_name'], 'mobile/uploads/blog_thumbnails/' . $data['blog_thumbnail']);
    move_uploaded_file($_FILES['blog_cover']['tmp_name'], 'mobile/uploads/blog_cover_images/' . $data['blog_cover']);
  }
  ...
  public function delete_blog($param1 = "")
  {
    ...
    unlink('mobile/uploads/blog_thumbnails/' . $thumbnail_image);
    unlink('mobile/uploads/blog_cover_images/' . $cover_image);
    $this->db->where('id', $param1);
  ...
  public function remove_listing_inner_feature()
  {
    ...
    if ($listing_type == 'food_menu') {
      ...
      unlink('mobile/uploads/restaurant_menu_images/' . $this->db->get_where($table_name, array('id' => $id))->row('photo'));
    } elseif ($listing_type == 'product') {
      $table_name = 'product_details';
      unlink('mobile/uploads/product_images/' . $this->db->get_where($table_name, array('id' => $id))->row('photo'));
    } elseif ($listing_type == 'hotel') {
      $table_name = 'hotel_room_specification';
      unlink('mobile/uploads/hotel_room_images/' . $this->db->get_where($table_name, array('id' => $id))->row('photo'));
    } elseif ($listing_type == 'beauty_service') {
      $table_name = 'beauty_service';
      unlink('mobile/uploads/beauty_service_images/' . $this->db->get_where($table_name, array('id' => $id))->row('photo'));
    }
    $this->db->where('id', $id);
    $this->db->delete($table_name);
    return true;
  ...
  function purchase_charge($method = "", $package_details = "", $user_details = "")
  {
    if ($method == 'stripe') {
      $user_details = $this->db->get_where('user', array('id' => $this->session->userdata('user_id')))->row_array();
      ...
      $checkout_session = \Stripe\Checkout\Session::create([
        ...
        'line_items' => [[
          'price_data' => [
            'currency' => get_settings('stripe_currency'),
            'unit_amount' => $package_details['price'] * 100,
            'product_data' => [
              'name' => $package_details['name'] . " Package",
              'images' => ['https://findmart.in/assets/global/light_logo.png'],
            ],
          ],
          'quantity' => 1,
        ]],
        'mode' => 'payment',
        'success_url' => $YOUR_DOMAIN . '/user/create_package_history/stripe/' . $package_details['id'] . '/' . $user_details['id'] . '/' . $package_details['price'],
        // 'success_url' => $YOUR_DOMAIN . 'user/create_package_history/stripe',
        // 'success_url' => $YOUR_DOMAIN . '/user/packages',
        'cancel_url' => $YOUR_DOMAIN . 'user/packages',
      ]);
  ...
  function create_package_purchase_history($payment_method = "", $user_id = "", $package_id = "", $paid_amount = "")
  {
    ...
    if ($this->purchase_charge($payment_method, $package_details, $user_details) == true || $package_details['package_type'] == 0) {
      $purchase_date          = strtotime(date('D, d-M-Y') . ' 00:00:00');
      $data['purchase_date']  = $purchase_date;
